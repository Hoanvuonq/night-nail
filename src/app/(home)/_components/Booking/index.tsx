...
import { SelectDateTime } from "../../../booking/_components/SelectDateTime";
...
const NightNailLuxury = ({ isOpen, onClose }: any) => {
  ...
  const currentServicePrice =
    SERVICES_DATA.find((s) => s.id === selectedService)?.price || "0đ";
      ...
      <div className="h-[88vh] flex flex-col relative bg-radial-at-t from-[#1A1A1A] to-[#0A0A0A]">
        <header className="sticky top-0 z-50 px-8 py-6 flex justify-between items-center border-b border-white/5 bg-black/60 backdrop-blur-xl">
          ...
          <div className="text-center">
            <h1 className="text-xl font-serif italic text-[#D4AF37] tracking-[0.2em] uppercase drop-shadow-[0_0_10px_rgba(212,175,55,0.3)]">
              Night Nail
            </h1>
            <span className="text-[8px] tracking-[0.4em] text-white/30 uppercase block mt-1">
                      ...
                      <AnimatePresence>
                        {isActive && (
                          <motion.div
                            initial={{ scale: 0 }}
                            animate={{ scale: 1 }}
                            className="bg-[#D4AF37] w-fit p-1 rounded-full text-black mb-2"
                          >
