...
const FormInput = ({ icon, name, placeholder, value, onChange, type = "text" }: any) => {
  ...
  return (
    <div className="group w-full">
      <div className={`relative flex rounded-[22px] border border-zinc-100 bg-zinc-50/50 p-1.5 transition-all duration-500 focus-within:border-amber-300 focus-within:bg-white focus-within:shadow-[0_10px_30px_-10px_rgba(251,191,36,0.2)] ${isTextarea ? "items-start" : "items-center"}`}>
        <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-[18px] bg-white text-amber-500 shadow-sm transition-transform duration-500 group-focus-within:scale-90 ${isTextarea ? "mt-1" : ""}`}>
          {icon}
        </div>
        {isTextarea ? (
          <textarea 
...
export const BookingSummaryForm = ({ currentService, selectedDate, selectedTime, formData, handleChange }: any) => {
  return (
    <motion.div
      ...
      exit={{ opacity: 0, y: -10 }}
      className="flex flex-1 flex-col p-6 md:px-10 space-y-7"
    >
      {/* --- PHẦN TÓM TẮT LỊCH HẸN (Luxury Card) --- */}
      <div className="relative overflow-hidden rounded-[32px] bg-linear-to-br from-zinc-900 to-zinc-800 p-5 shadow-xl">
        ...
        <div className="relative z-10 space-y-4">
          ...
          <div className="grid grid-cols-1 gap-4">
            ...
            <div className="flex items-center gap-4">
              <div className="relative h-14 w-14 shrink-0 overflow-hidden rounded-[20px] border border-white/10">
                <Image 
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-xs font-bold text-zinc-200">{selectedDate}</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  ...
                  <p className="text-xs font-bold text-zinc-200">{selectedTime}</p>
                </div>
              </div>
            </div>
          </div>
      ...
      <div className="flex-1 space-y-6">
        <div className="flex flex-col items-center text-center space-y-1">
          <div className="flex items-center gap-2 text-amber-600">
             ...
             <span className="text-[10px] font-black uppercase tracking-widest">Xác nhận nàng nhé</span>
             <div className="h-px w-8 bg-amber-200" />
          </div>
          <h3 className="font-serif text-2xl text-zinc-900 italic">Để lại lời nhắn cho em</h3>
        </div>
