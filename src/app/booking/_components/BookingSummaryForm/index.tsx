...
import { CalendarIcon, Clock, User, Phone, Edit3, Sparkles, ShieldCheck, Heart } from "lucide-react";
...
const FormInput = ({ icon, name, placeholder, value, onChange, type = "text" }: any) => {
  ...
  return (
    <motion.div variants={itemVariants} className="group w-full">
      <div className={`relative flex rounded-3xl border border-zinc-100 bg-white p-1.5 transition-all duration-300 focus-within:border-amber-300 focus-within:shadow-[0_10px_30px_-10px_rgba(251,191,36,0.15)] ${isTextarea ? "items-start" : "items-center"}`}>
        <div className={`flex h-11 w-11 shrink-0 items-center justify-center rounded-[20px] bg-amber-50 text-amber-500 transition-colors duration-300 group-focus-within:bg-amber-500 group-focus-within:text-white ${isTextarea ? "mt-1" : ""}`}>
          {icon}
        </div>
        {isTextarea ? (
          <textarea 
  ...
  return (
    ...
    >
      ...
      >
        ...
        <div className="relative z-10 space-y-4">
          <div className="flex items-center justify-between">
            <p className="text-[10px] font-bold text-amber-400 uppercase tracking-[0.2em]">Thông tin đặt lịch</p>
          ...
          <div className="flex items-center gap-4">
            ...
            <div className="min-w-0 flex-1">
              <h4 className="font-serif text-lg italic text-white truncate">{currentService.title}</h4>
              <p className="text-xs font-bold text-amber-400 flex items-center gap-1.5">
                  <Sparkles size={10} fill="currentColor" /> {currentService.price}
              </p>
            </div>
          </div>
      ...
      <div className="flex-1 space-y-5">
        <motion.div variants={itemVariants} className="flex flex-col items-center text-center space-y-1">
          <div className="flex items-center gap-2 text-amber-600">
             <div className="h-px w-8 bg-amber-200" />
             <span className="text-[14px] font-bold uppercase">Xác nhận nàng nhé</span>
             <div className="h-px w-8 bg-amber-200" />
          </div>
          <h3 className="text-2xl text-zinc-900 italic">Để lại lời nhắn cho em</h3>
        </motion.div>
      ...
      <motion.div variants={itemVariants} className="pb-2 text-center space-y-4">
        ...
        <p className="text-[10px] text-zinc-300 font-bold uppercase tracking-[0.2em] flex items-center justify-center gap-2">
           <Heart size={10} fill="currentColor" className="text-pink-300" /> Hẹn gặp nàng sớm nha
        </p>
      </motion.div>
    </motion.div>
  );
